...
for source, target, weight in edges_data:
    ...
    arrow_trace = go.Scatter(
        ...
        hovertext=f'{source} → {target}<br>Confidence: {weight:.4f}',
        showlegend=False
    )
...
fig = go.Figure(data=[node_trace] + edge_traces,
               layout=go.Layout(
                ...
                xaxis=dict(
                    ...
                    fixedrange=False  # Enables zoom on x-axis
                ),
                yaxis=dict(
                    showgrid=False, 
                    zeroline=False, 
                    showticklabels=False,
                    fixedrange=False  # Enables zoom on y-axis
                ),
                plot_bgcolor='white',
                width=1200,
                height=800
...
# pos = circular_layout(nodes)
pos = nx.spring_layout(G, k=0.7, iterations=100)  # Added: spring layout parameters
